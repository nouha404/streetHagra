...
let isPlayerReady = false; // Indique si le joueur est prêt
let countdown = 3; // Compte à rebours initial
...
confirmMapButton.addEventListener("click", () => {
    ...
    alert(`Vous avez choisi la carte : ${selectedMap}`);
    ...
    showWaitingMessage(); // Afficher le message d'attente
});

// Écouter l'événement indiquant que les deux joueurs ont choisi leur carte
socket.on("bothMapsSelected", () => {
    // Masquer le message d'attente
    const waitingMessage = document.getElementById("waiting-message");
    if (waitingMessage) {
        waitingMessage.remove(); // Supprimer le message d'attente
    }
...
function showWaitingMessage() {
    ...
    waitingMessage.textContent = "En attente du joueur 2...";
...
let isCountdownFinished = false; // Variable pour bloquer/débloquer les mouvements
let isCountdownStarted = false;  // Variable pour empêcher le compte à rebours de se relancer

function startCountdown() {
    // Vérifier si le compte à rebours a déjà été lancé
    if (isCountdownStarted) {
        console.log("Le compte à rebours a déjà été lancé.");
        return;
    }

    isCountdownStarted = true; // Marquer le compte à rebours comme lancé
    ...
    const countdownImages = [
        '/assets/img/countdown/3.png', // Image pour 3
        '/assets/img/countdown/2.png', // Image pour 2
        '/assets/img/countdown/1.png'  // Image pour 1
    ];
    ...
    const interval = setInterval(() => {
        if (currentIndex >= countdownImages.length) {
            ...
            countdownElement.remove(); // Supprimer l'élément du compte à rebours

            // Débloquer les mouvements
            isCountdownFinished = true;
            console.log("Le compte à rebours est terminé. Les joueurs peuvent bouger.");
            return;
        }

        // Afficher l'image correspondante
        countdownElement.innerHTML = `<img src="${countdownImages[currentIndex]}" alt="Countdown" style="width: 200px; height: auto;">`;
        currentIndex++;
    }, 1000); // Changer d'image toutes les secondes
}


// Écouter l'événement de compte à rebours
...
socket.on("updatePlayers", (data) => {
    ...
    const playersReady = Object.values(players).filter(player => player.ready).length;
    if (playersReady === 2) {
        // Masquer le message d'attente immédiatement
        const waitingMessage = document.getElementById("waiting-message");
        if (waitingMessage) {
            waitingMessage.remove(); // Supprimer le message d'attente
        }

        // Lancer le compte à rebours après avoir masqué le message
        startCountdown();
    }
});
...
document.addEventListener("keydown", (e) => {
    if (isGameOver || !isCountdownFinished) return; // Bloquer les mouvements si le compte à rebours n'est pas terminé
